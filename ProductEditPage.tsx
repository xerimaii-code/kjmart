...
export default function ProductEditPage({ isOpen, onClose, initialBarcode }: ProductEditPageProps) {
    ...
    // Update supplierSearch when comcode is set programmatically
    useEffect(() => {
    ...
    const populateProductData = useCallback(async (p: any) => {
        ...
        setIsUse(String(p.isuse || p.사용유무) === '1');
        setIsTaxable(String(p.isvat || p.과세여부) === '1');
        setIsPoint(String(p.ispoint || p.포인트적립) === '1');
        setIsStockManaged(String(p.isjago || p.재고관리) === '1');
        ...
        if (isPack) {
            // [중요 수정] 사용자 정의 쿼리 'BOM'의 SQL 구문을 직접 찾아 전달하여 실행
            ...
            if (bomQueryObj) {
                ...
                console.warn("'BOM' 쿼리가 SQL Runner에 등록되지 않았습니다.");
                setBomList([]);
            }
        } else {
            setBomList([]);
                            ...
                            <div className="relative">
                                ...
                                {comcode ? (
                                    <button 
                                        ...
                                        className="absolute right-1 top-1/2 -translate-y-1/2 p-1 text-gray-400 hover:text-rose-500"
                                        type="button"
                                    >
                            ...
                            <div className="p-1 flex-1 overflow-y-auto">
                                {bomList.length > 0 ? (
                                    <div className="space-y-1">
                                        {bomList.map((item, idx) => (
                                            <div key={idx} className="bg-white border border-gray-100 rounded p-1.5 shadow-sm flex flex-col gap-0.5">
                                                ...
                                                <p className="text-xs font-bold text-gray-800 leading-tight truncate">{item.상품명}</p>
                                                <div className="flex justify-between items-center mt-0.5 border-t border-gray-50 pt-0.5">
                                                    <p className="text-[10px] text-gray-600 font-medium">{Number(item.매입가).toLocaleString()}원</p>
                                    ...
                                    <div className="h-full flex items-center justify-center">
                                        <p className="text-sm text-gray-400">{isBundle ? 'BOM 로딩 중...' : '일반 상품'}</p>
                                    </div>
                                )}
                            </div>
                        </div>

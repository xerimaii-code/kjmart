...
const NAMED_QUERIES = {
  searchProductsForEdit: `
    SELECT TOP 50
        ...
        p.isuse  AS [사용유무],
        -- Calculate margin rate dynamically as column might not exist or be unreliable
        CASE 
            WHEN p.money1 > 0 THEN ((p.money1 - p.money0vat) / p.money1) * 100 
            ELSE 0 
        END AS [이익율],
        p.ispoint AS [고객점수가산],
        p.isjago AS [재고관리여부]
  ...
  행사목록: `
    SELECT 
        ...
        , A.startday    AS [시작일]
        , A.endday      AS [종료일]
        , A.itemcount   AS [행사품목수] 
        , A.isappl      AS [상태]       
        , A.avgmgrate   AS [평균마진율]
        , A.appendday   AS [등록일자]
        , A.appendid    AS [등록자ID]
    FROM sale_mast A WITH(NOLOCK)
    WHERE A.endday >= @startDate
      AND (@status = '' OR A.isappl = @status)
    ORDER BY A.startday DESC, A.junno DESC
  `,
  행사상세: `
    SELECT
        CASE
            WHEN s.isappl = '1' THEN '적용'
            WHEN s.isappl = '2' THEN '완료'
            ELSE '미적용'
        END AS [적용상태],
        s.barcode AS [바코드],
        p.descr AS [상품명],
        ISNULL(p.spec, '') AS [규격],
        ISNULL(s.salemoney0, 0) AS [행사매입],
        ISNULL(s.salemoney1, 0) AS [행사판가],
        ISNULL(p.money1, 0) AS [이전판가],
        ISNULL(c.comname, '') AS [거래처명]
    FROM sale_ready AS s WITH(NOLOCK)
    LEFT JOIN parts AS p WITH(NOLOCK) ON s.barcode = p.barcode
    LEFT JOIN comp AS c WITH(NOLOCK) ON p.comcode = c.comcode
    WHERE s.junno = @junno
    ORDER BY p.descr
  `,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'syncBOM': {
        const result = await pool.request().query(bomQuery);
        res.status(200).json({ recordset: result.recordset });
        break;
      }

      case 'syncCategories': {
        const [gubun1Result, gubun2Result, gubun3Result] = await Promise.all([
          pool.request().query(gubun1Query),
          pool.request().query(gubun2Query),
          pool.request().query(gubun3Query),
        ]);

...
import { Order, OrderItem, Customer, Product, DeviceSettings, SyncLog } from '../types';
...
export const subscribeToSavedQueries = (callback: (queries: SavedQuery[]) => void): (() => void) => {
    if (!db) {
        callback([]);
        return () => {};
    }
...
export const addSavedQuery = async (queryData: Omit<SavedQuery, 'id'>): Promise<string> => {
    if (!db) throw DB_UNAVAILABLE_ERROR;
    const queriesRef = ref(db, 'saved-queries');

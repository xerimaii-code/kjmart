...
const DEFAULT_TIMEOUT_MS = 30000; // Keep query timeout robust

async function fetchApi(body: object, signal?: AbortSignal) {
    let timeoutId: any = null;
    let effectiveSignal = signal;

    // 호출자가 signal을 제공하지 않은 경우, 기본 타임아웃을 적용한 signal 생성
    if (!effectiveSignal) {
    ...
    try {
        ...
        if (err.name === 'AbortError') {
            if (signal?.aborted) {
                ...
                // 내부 타임아웃에 의한 중단
                throw new Error('서버 응답 시간이 초과되었습니다. 연결을 확인해주세요.');
            }
        }
...
export async function checkSqlConnection(): Promise<{ success: boolean; message: string }> {
    const controller = new AbortController();
    const timeout = 3000; // Reverted to 3 seconds as requested

...
import { getDraftBatchesCount } from '../services/receiveDbService';
import { getDraft } from '../services/draftDbService';
import { SW_VERSION } from '../config';
import { ReceivingDraft } from '../types';
...
const MenuButton = memo(({ label, icon, onClick, subText, className, badge }: MenuButtonProps) => {
    return (
        <button 
            onClick={onClick} 
            className={`w-full px-2 py-2 rounded-xl border border-slate-100 bg-white flex items-center justify-between active:scale-[0.98] transition-all hover:shadow-md hover:border-slate-200 group ${className || ''}`}
        >
...
const MenuPage: React.FC<MenuPageProps> = ({ onNavigate }) => {
    ...
    const [receivingDraftCount, setReceivingDraftCount] = useState(0);
    const [hasLocalReceivingDraft, setHasLocalReceivingDraft] = useState(false);
    ...
    useEffect(() => {
        ...
        const checkDrafts = async () => {
            try {
                const count = await getDraftBatchesCount();
                setReceivingDraftCount(count);

                const localDraft = await getDraft<ReceivingDraft>('receiving-new-draft');
                setHasLocalReceivingDraft(!!localDraft && (localDraft.items.length > 0 || !!localDraft.selectedSupplier));
            } catch (e) {
        ...
        checkDrafts();
        const intervalId = setInterval(checkDrafts, 5000);
        ...
        <div className="absolute inset-0 bg-slate-50 flex flex-col overflow-hidden">
            <div className="flex-grow flex flex-col px-3 py-3 overflow-y-auto">
                <div className="max-w-7xl mx-auto w-full pb-safe">
                    ...
                    {/* 1. Dashboard (Full Width on all screens) */}
            ...
            <div className="flex items-center justify-between px-4 py-2.5 flex-shrink-0 safe-area-pb bg-white/90 backdrop-blur-sm border-t border-slate-100 z-10 relative">
                <button
                    type="button"

...
import { SpinnerIcon, TrashIcon, CheckCircleIcon, BarcodeScannerIcon } from '../components/Icons';
...
const InventoryAuditPage: React.FC<{ isActive: boolean }> = ({ isActive }) => {
    ...
    const [applyMode, setApplyMode] = useState<'immediate' | 'batch'>('batch');
    ...
    useEffect(() => {
        if (isActive && draft && auditedItems.length === 0) {
            ...
            setApplyMode(draft.applyMode || 'batch');
        }
                ...
                if (res && res[0]?.RESULT === 'SUCCESS') {
                    ...
                    setAuditedItems(prev => {
                        const newItem: AuditedItem = {
                            ...
                            name: p.name,
                            spec: p.spec,
                            computerStock: Number(res[0].OLD_QTY),
                            auditQty: auditQty,
                            diff: Number(res[0].DIFF_QTY),
                            timestamp: Date.now()
                        };
        ...
        <div className="h-full flex flex-col bg-gray-50 overflow-hidden">
            <div className="p-3 bg-white border-b border-gray-200 flex-shrink-0 z-20 shadow-sm">
                <ProductSearchBar 
                    ...
                    isSearching={isSearching || isFetchingStock}
                    results={results}
                    onSelectProduct={(p) => {
                        setSearchTerm('');
                    ...
                    onBoxUnitChange={() => {}}
                />
                <div className="flex bg-gray-200 rounded-xl p-1 mt-3 shadow-inner">
                    <button onClick={() => setApplyMode('batch')} className={`flex-1 py-2.5 rounded-lg text-sm font-black transition-all duration-200 ${applyMode === 'batch' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-300'}`}>일괄 적용</button>
                    <button onClick={() => setApplyMode('immediate')} className={`flex-1 py-2.5 rounded-lg text-sm font-black transition-all duration-200 ${applyMode === 'immediate' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-500 hover:bg-gray-300'}`}>즉시 적용</button>
                </div>
            </div>
            ...
            <div className="p-3 bg-white border-t border-gray-200 safe-area-pb shadow-lg z-20">
                ...
                <div className="grid grid-cols-3 gap-2">
                    ...
                    <button 
                        ...
                        disabled={isSaving || auditedItems.length === 0}
                    ...
                    >
                        {isSaving ? <SpinnerIcon className="w-6 h-6 animate-spin" /> : (
                            applyMode === 'batch' ? <><CheckCircleIcon className="w-5 h-5"/> 일괄 전송</> : '즉시 반영됨'
                        )}
                    </button>
                </div>
            </div>
            ...
            {auditModalProps.isOpen && (
                <StockAuditItemModal 
                    ...
                    onClose={() => setAuditModalProps(p => ({ ...p, isOpen: false }))}
                    onConfirm={(qty, nextScan) => { handleAddAudit(qty, nextScan); setAuditModalProps(p => ({ ...p, isOpen: false })); }}

...
const OrderItemRow = memo(({ item, product, onEdit, onRemove, index, isDragging, onDragStart, onDragEnd, onDragOver, onDrop }: { 
    ...
    return (
        <div
            ...
            onClick={() => onEdit(item)}
        >
            {/* Drag Handle Icon for visual hint */}
...
const NewOrderPage: React.FC<{ isActive: boolean }> = ({ isActive }) => {
    ...
    const { setLastModifiedOrderId, activeMenuOrderId } = useMiscUI();
    ...
    // [FIX] Add missing filteredCustomers to handle customer searching in the dropdown.
    ...
    }, [addOrder, setLastModifiedOrderId, showAlert, removeDraftData, showToast, resetItems, setProductSearch]);
                            ...
                            return (
                                <React.Fragment key={item.barcode}>
                                    ...
                                    <OrderItemRow 
                                        item={item} 
                                        product={product} 
                                        onEdit={(it) => openEditItemModal({ item: it, onSave: (det) => updateItem(it.barcode, { ...det, isModified: true }), onScanNext: handleOpenScanner })} 

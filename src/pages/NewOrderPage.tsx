...
const NewOrderPage: React.FC<{ isActive: boolean }> = ({ isActive }) => {
    ...
    return (
        <div className="h-full flex flex-col relative bg-white">
            ...
            {/* Top Loading Bar */}
            {isSearchingProducts && (
                <div className="fixed top-0 left-0 w-full h-1 bg-indigo-100 z-[100] overflow-hidden">
                    <div className="h-full bg-indigo-600 animate-[indeterminate_1.5s_infinite_linear] origin-left"></div>
                </div>
            )}
            <style>{`
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className={`w-full px-3 h-11 border ${isCustomerSelected ? 'border-indigo-500 bg-indigo-50/50 pr-28 font-semibold text-indigo-800' : 'border-gray-300 bg-white pr-10'} rounded-xl focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 transition-colors duration-200 text-base`}
                                autoComplete="off"
                            />
                            ...
                            {!isCustomerSelected && (
                                <button
                                    ...
                                    className="absolute top-1/2 right-1 -translate-y-1/2 p-2 text-gray-400 hover:text-indigo-600 transition-colors"
                                    type="button"
                                    aria-label="목록 보기"
                                >
                             ...
                             {isCustomerSelected && (
                                <button
                                    ...
                                    className="absolute top-1/2 right-1.5 -translate-y-1/2 h-8 px-3 rounded-lg flex items-center justify-center gap-1.5 font-semibold transition bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 active:scale-95 text-sm"
                                    aria-label="거래처 변경"
                                >
                                    변경
                                </button>
                            ...
                            <SearchDropdown<Customer>
                                ...
                                renderItem={(c) => (
                                    <div onMouseDown={(e) => { e.preventDefault(); handleSelectCustomer(c); }} className="p-3 hover:bg-gray-50 cursor-pointer border-b last:border-b-0">
            ...
            {(items.length > 0 || selectedCustomer) && (
                 <footer className="p-3 bg-white border-t border-gray-100 z-10 flex-shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div className="max-w-2xl mx-auto">
                        <div className="flex justify-between items-center font-bold mb-3 px-2">
                            ...
                            <span className="text-xl text-gray-900 tracking-tighter">{totalAmount.toLocaleString()} 원</span>
                        </div>
                        <div className="grid grid-cols-3 gap-3">
                             <button
                                onClick={handleResetOrder}
                                className="h-12 bg-gray-100 text-gray-600 rounded-xl font-bold text-base hover:bg-gray-200 transition shadow-sm flex items-center justify-center active:scale-95"
                            >
                                초기화
                            </button>
                            <button
                                ...
                                className="relative col-span-2 h-12 bg-indigo-600 text-white rounded-xl font-bold text-base hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/30 disabled:bg-gray-300 disabled:text-gray-500 disabled:shadow-none disabled:cursor-not-allowed flex items-center justify-center active:scale-95"
                            >

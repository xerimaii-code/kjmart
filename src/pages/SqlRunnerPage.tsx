...
const QUERY_TEMPLATES = [
    {
        ...
        sql: `-- [묶음 상품(BOM) 구성 품목 조회]\n-- 파라미터: @barcode (부모 상품 바코드)\n\nSELECT \n    b.ccode AS [바코드], \n    p.descr AS [상품명], \n    p.spec  AS [규격], \n    p.money0vat AS [매입가], \n    b.childcount AS [수량]\nFROM bom AS b WITH(NOLOCK)\nJOIN parts AS p WITH(NOLOCK) ON b.ccode = p.barcode\nWHERE b.pcode = @barcode`
    },
    {
        name: "상품조회 (행사포함)",
        description: "현재 진행 중인 행사 정보를 포함하여 상품을 검색합니다.",
        sql: `DECLARE @Today VARCHAR(10) = CONVERT(VARCHAR(10), GETDATE(), 120);\n\nSELECT TOP 50 \n  p.barcode AS [바코드], \n  p.descr AS [상품명], \n  p.money1 AS [판매가], \n  CASE WHEN s.barcode IS NOT NULL THEN 'Y' ELSE 'N' END AS [행사유무],\n  ISNULL(s.salemoney1, 0) AS [행사판매가],\n  ISNULL(m.salename, '') AS [행사명]\nFROM parts p WITH(NOLOCK)\nLEFT JOIN sale_ready s WITH(NOLOCK) ON p.barcode = s.barcode AND s.isappl = '1'\nLEFT JOIN sale_mast m WITH(NOLOCK) ON s.junno = m.junno AND m.isappl = '1' AND (@Today BETWEEN m.startday AND m.endday)\nWHERE p.descr LIKE '%@kw%' OR p.barcode = '@kw'`
    },
    {
        name: "시간대별 매출 (속보용)",
        description: "오늘의 시간대별 총 매출액을 집계합니다.",
        sql: "SELECT SUBSTRING(CONVERT(VARCHAR(8), sale_time, 108), 1, 2) AS [시간], SUM(tot_sale_amt) AS [매출액] FROM sale_header WITH(NOLOCK) WHERE sale_date = CONVERT(VARCHAR(10), GETDATE(), 120) GROUP BY SUBSTRING(CONVERT(VARCHAR(8), sale_time, 108), 1, 2) ORDER BY [시간]"
    },
    {
        name: "행사 목록 조회 (진행중)",

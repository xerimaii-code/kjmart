...
import { DevicePhoneMobileIcon, LogoutIcon, DatabaseIcon, ShieldCheckIcon, SpinnerIcon, CloudArrowDownIcon, CheckSquareIcon, CancelSquareIcon, HistoryIcon, TrashIcon } from '../components/Icons';
import ToggleSwitch from '../components/ToggleSwitch';
import { getReceivingBatchesByDateRange, clearSyncLogs as dbClearSyncLogs } from '../services/dbService';
...
const SettingsPage: React.FC<SettingsPageProps> = ({ isActive }) => {
    ...
    const handleClearSyncHistory = () => {
        showAlert(
            "모든 동기화 이력을 삭제하시겠습니까?\n이 작업은 되돌릴 수 없으며, 실제 데이터에는 영향을 주지 않습니다.",
            async () => {
                try {
                    await dbClearSyncLogs(); 
                    showToast("동기화 이력이 삭제되었습니다.", "success");
                } catch (err) {
                    showAlert("동기화 이력 삭제에 실패했습니다.");
                }
            },
            '삭제',
            'bg-rose-500 hover:bg-rose-600 focus:ring-rose-500'
            ...
            <div className="scrollable-content p-4">
                <div className="max-w-xl mx-auto w-full">
                        ...
                        <SettingsRow label="스캔 성공 시 효과음">
                         ...
                         <SettingsRow label="키패드 터치음">
                            <ToggleSwitch 
                                id="sound-on-press"
                                label=""
                                checked={!!uiFeedback.soundOnPress}
                                onChange={(checked) => setUiFeedback({ soundOnPress: checked })}
                                color="blue"
                            />
                        </SettingsRow>
                    ...
                    <SettingsSection title="데이터 관리">
                        ...
                        <SettingsRow label="동기화 이력 보기" subLabel="최근 데이터 동기화 기록" onClick={() => setIsSyncHistoryModalOpen(true)}>
                            <HistoryIcon className="w-6 h-6 text-gray-500" />
                        </SettingsRow>

                        <SettingsRow label="동기화 이력 초기화" subLabel="모든 동기화 기록 삭제" onClick={handleClearSyncHistory} isDestructive={true}>

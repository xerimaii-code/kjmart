...
import { addReceivingBatch, listenToReceivingBatchChanges, cleanupOldReceivingBatches, deleteReceivingBatch } from '../services/dbService';
...
const ReceiveManagerPage: React.FC<ReceiveManagerPageProps> = ({ isActive, onClose }) => {
    ...
    // Refs for Strict Data Safety (Prevent Stale Closures)
    ...
    useEffect(() => {
        ...
        receiveDb.cleanupOldLocalBatches(2);
    }, []);
    ...
    const executeBatchSend = async () => {
        ...
        const safeInsertQuery = `; INSERT INTO dbo.dt900_ipgo (day1, dtcomcode, comcode, comname, barcode, descr, money0vat, money1, itemcount, gubun, lstmoney0vat) SELECT @time, @dtcomcode, @dtcomcode, LEFT(@comname, 10), @barcode, LEFT(ISNULL(@item_name, ''), 30), @cost, @price, @qty, CASE WHEN CAST(@qty AS INT) >= 0 THEN 'I' ELSE 'B' END, ISNULL(p.money0vat, 0) FROM (SELECT 1 AS dummy) AS t LEFT JOIN dbo.parts AS p WITH (NOLOCK) ON p.barcode = @barcode`;
        const now = new Date();
        const unifiedTime = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`; 
        ...
        for (const batch of batchesToSend) {
            ...
            try {
                for (const item of batch.items) {
                    const params = {
                        ...
                        qty: item.quantity, cost: item.costPrice, price: item.sellingPrice,
                    ...
                    } catch (itemErr) { sqlSuccess = false; break; }
                }
            } catch (err: any) { sqlSuccess = false; }
            ...
            if (sqlSuccess) {
                ...
                } catch (fbErr) { showToast('SQL 저장은 성공했으나, 클라우드 백업에 실패했습니다.', 'error'); }
            } else {
        ...
        setSelectedBatches(prev => { const next = new Set(prev); successIds.forEach(id => next.delete(id)); return next; });
        setIsSending(false);
        refreshLocalBatches(true);
    ...
    const setupNewBatchState = () => {
        ...
        setBatchDate(new Date().toISOString().slice(0, 10)); setCurrentItems([]);
    };
    ...
    const handleResumeDraft = () => {
        ...
        setBatchDate(draft.currentDate || new Date().toISOString().slice(0, 10));
    ...
    // Robust Add Item with Immediate Draft Save
    ...
    const onScanDetected = async (code: string) => {
        try {
            ...
            if (!product) {
                ...
                product = await searchByBarcode(code);
            }
    ...
    const handleScanButtonClick = () => {
        openScanner(
            'modal', // Context
            onScanDetected, 
            { continuous: true } // Enable Continuous Scan
        );
    };
        ...
        <>
            <div className="flex flex-col h-full bg-white">
                <div className="bg-white p-3 border-b flex justify-between items-center shadow-sm z-10">
                    ...
                    <div className="flex gap-2">
                        {isBackgroundSyncing && <SpinnerIcon className="w-5 h-5 text-indigo-500 animate-spin self-center" />}
                        {hasDraft ? (
                                            ...
                                            <div onClick={() => toggleAccordion(batch.id)} className="flex-grow flex flex-col justify-center py-3 pr-4 pl-1 cursor-pointer active:bg-gray-50 transition-colors min-w-0">
                                                <div className="flex justify-between items-center mb-1">
                                                    ...
                                                    <div className="flex items-center gap-2 flex-shrink-0">
                                                        {batch.status === 'draft' && !isBackgroundSyncing && <span className="text-[10px] text-amber-600 font-bold border border-amber-200 bg-amber-50 px-1 rounded">미전송</span>}
                ...
                {selectedBatches.size > 0 && (
                    <div className="p-3 bg-white border-t safe-area-pb shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
                        <button onClick={handleSend} disabled={isSending} className="w-full bg-indigo-600 text-white py-3.5 rounded-xl font-bold text-lg shadow-lg active:scale-95 disabled:bg-gray-400 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-colors">
                            {isSending ? <SpinnerIcon className="w-5 h-5" /> : <CheckCircleIcon className="w-6 h-6" />}
                ...
                <div className="flex flex-col h-full bg-white">
                    ...
                    <div className="p-3 border-t bg-white safe-area-pb shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
                        <div className="flex justify-between items-center mb-3 px-1">
                            ...
                            <div className="flex items-end gap-1"><span className={`text-lg font-bold tracking-tight ${totalAmount < 0 ? 'text-rose-600' : 'text-indigo-600'}`}>{totalAmount.toLocaleString()}</span><span className="text-sm font-bold text-gray-600 mb-0.5">원</span></div>
                        </div>
            ...
            {/* Item Input Modal (Overlay on Global Scanner) */}
            {addItemModalProps.isOpen && (
                <ReceiveItemModal
                    key={addItemModalProps.scanTimestamp}
                    isOpen={true}
                    ...
                    timestamp={addItemModalProps.scanTimestamp} // Pass scanTimestamp prop explicitly
                />
            )}
        </>
    );

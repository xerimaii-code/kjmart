...
const ORDER_ROW_HEIGHT = 88; // Estimated height for an order row
const DATE_HEADER_HEIGHT = 48; // Estimated height for a date header
...
const OrderHistoryPage: React.FC<OrderHistoryPageProps> = ({ isActive }) => {
    ...
    const { virtualItems, totalHeight } = useMemo(() => {
        ...
        groupedOrders.forEach(group => {
            items.push({ type: 'header', height: DATE_HEADER_HEIGHT, top: currentHeight, data: group });
            currentHeight += DATE_HEADER_HEIGHT;
            group.orders.forEach(order => {
                items.push({ type: 'order', height: ORDER_ROW_HEIGHT, top: currentHeight, data: order });
    ...
    const handleCardClick = useCallback(async (e: React.MouseEvent) => {
        ...
        // Always fetch fresh data before opening the detail modal
        // to prevent showing stale data from the list view state,
        // which can happen after the app resumes from background.
        try {
                ...
                {isLoading ? (
                    ...
                    <div className="max-w-2xl mx-auto w-full relative" style={{ height: `${totalHeight}px` }}>
                        {visibleItems.map(item => (
                            <div
                                key={item.type === 'header' ? item.data.date : item.data.id}
                                className="absolute w-full px-1"
                                style={{ top: `${item.top}px`, height: `${item.height}px` }}
                            >
                                {item.type === 'header' ? (
                                    <div className="bg-gray-50/95 backdrop-blur-sm py-2 flex justify-between items-center mb-1">
                                        <h3 className="font-bold text-gray-600 text-sm">
                                            {new Date(item.data.date + 'T00:00:00').toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' })}
                                        </h3>
                                        <span className="font-bold text-gray-800 text-sm">{item.data.total.toLocaleString()} 원</span>
                                    </div>
                                ) : (
                                    <div className="py-1">
                                        <OrderRow
                                            order={item.data}
                                            isHighlighted={item.data.id === lastModifiedOrderId}
                                            hasDraft={draftKeys.has(item.data.id)}
                                            isMenuOpen={activeMenuOrderId === item.data.id}

...
const AddItemModal: React.FC<AddItemModalProps> = ({ isOpen, product, existingItem, onClose, onAdd, onNextScan, trigger, initialSettings, timestamp }) => {
    ...
    // We use product directly instead of syncing local activeProduct state
    ...
    // Critical: Initialize as true to ensure first keypress clears the default '1'
    ...
    // Force Reset on Open/Product Change/Timestamp Change
    useEffect(() => {
        if (isOpen && product) {
            setQuantity(1);
    ...
    // --- Section 1: Controller (Keypad + Actions) ---
    const ControllerSection = (
        <div className="flex flex-col gap-1 h-full w-full min-w-0">
            ...
            <div className="flex flex-col gap-1 flex-shrink-0 mb-1">
                {trigger === 'scan' ? (
                    <>
                        {/* Scan Mode: Row 1 - Skip & Close */}
                        <div className={`grid gap-1 ${isLeftHanded ? 'grid-cols-[1fr_1fr]' : 'grid-cols-[1fr_1fr]'}`}>
                            {isLeftHanded ? (
                                <>
                                    <ButtonBase onClick={handleClose} className="bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200 py-2.5 text-xs">
                                        <XCircleIcon className="w-4 h-4 mr-1" />
                                        취소
                                    </ButtonBase>
                                    <ButtonBase onClick={handleSkip} className="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 py-2.5 text-xs">
                        ...
                        {/* Scan Mode: Row 2 - Save (Renamed from Add & Close) */}
                        <ButtonBase 
                            onClick={handleAdd} 
                            disabled={!isQuantityValid} 
                            className="bg-white border border-blue-200 text-blue-700 hover:bg-blue-50 font-bold py-2 text-xs shadow-sm h-10"
                        >
                            <SaveIcon className="w-3.5 h-3.5 mr-1" />
                            저장 & 종료
                        </ButtonBase>

                        {/* Scan Mode: Row 3 - Add & Scan */}
                        <ButtonBase 
                            onClick={handleAddAndScan} 
                            disabled={!isQuantityValid}
                            className="w-full shadow-md bg-blue-600 hover:bg-blue-700 text-white py-4 text-base font-bold h-14"
                        >
                            <BarcodeScannerIcon className="w-5 h-5 mr-1.5" />
                            추가 & 스캔
                        </ButtonBase>
                    </>
                ) : (
                    <>
                        {/* Search Mode: Cancel (Top), Save (Bottom) */}
    ...
    const InfoSection = (
        <div className="flex flex-col h-full w-full pl-1 overflow-y-auto min-w-0">
            ...
            {/* Prices (Updated Layout: Removed Stock) */}
            <div className="flex items-center justify-between bg-gray-50 px-2 py-1.5 rounded text-xs text-gray-600 mb-2 flex-shrink-0 border border-gray-100">
                <div className="flex gap-3 w-full justify-around">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <b className="text-gray-900">{product.sellingPrice?.toLocaleString()}</b>
                    </div>
                </div>
            </div>

            {/* Quantity Area (Label + Input + Info below) */}
            <div className="flex-grow flex flex-col min-h-0 mb-1">
                {/* Existing Quantity Row - Full Width & Compact */}
                {existingItem && (
                ...
                >
                    ...
                    <span className="text-2xl font-extrabold text-gray-900 tracking-tight z-10">
                        {quantity}
                    </span>
                </div>

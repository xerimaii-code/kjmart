                ...
                <div className="p-5">
                    ...
                    <div className="text-center text-gray-600 mb-4 space-y-1">
                        <div className="text-base flex items-baseline justify-center gap-x-2 flex-wrap">
                            ...
                            <span className="text-sm text-gray-500">판매:</span>
                ...
                <div className="bg-gray-50 px-4 py-3 rounded-b-xl">
                    {isContinuousScan ? (
                        <div className="space-y-2">
                             <button onMouseDown={(e) => e.preventDefault()} onClick={handleAddAndScan} disabled={!isQuantityValid} className="w-full text-white px-4 h-14 flex items-center justify-center rounded-lg font-bold text-lg bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition active:scale-95 disabled:bg-gray-400 disabled:cursor-not-allowed shadow-md shadow-blue-500/30">추가 후 계속 스캔</button>
                            <div className="grid grid-cols-3 gap-2">
                                <button onMouseDown={(e) => e.preventDefault()} onClick={onClose} className="px-2 h-12 flex items-center justify-center rounded-md font-semibold text-gray-600 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-sm active:scale-95">스캔 종료</button>
                                <button onMouseDown={(e) => e.preventDefault()} onClick={handleAdd} disabled={!isQuantityValid} className="px-2 h-12 flex items-center justify-center rounded-md font-semibold text-blue-600 bg-blue-100 hover:bg-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-400 transition text-sm active:scale-95 text-center disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed">추가 후 종료</button>
                                <button onMouseDown={(e) => e.preventDefault()} onClick={() => { if (onNextScan) { onClose(); onNextScan(); } }} className="px-2 h-12 flex items-center justify-center rounded-md font-semibold text-gray-600 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-sm active:scale-95">건너뛰기</button>
                            </div>
                        </div>
                    ) : (
                        <div className="grid grid-cols-2 gap-3">
                            <button onMouseDown={(e) => e.preventDefault()} onClick={onClose} className="px-4 h-10 flex items-center justify-center rounded-lg font-semibold text-gray-700 bg-gray-200 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400 transition text-base active:scale-95">취소</button>
                            <button onMouseDown={(e) => e.preventDefault()} onClick={handleAdd} disabled={!isQuantityValid} className="text-white px-4 h-10 flex items-center justify-center rounded-lg font-bold bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition text-base active:scale-95 disabled:bg-gray-400 disabled:cursor-not-allowed">{existingItem ? '수량 추가' : '품목 추가'}</button>
                        </div>
                    )}
                </div>
            </div>

...
import { XCircleIcon, SaveIcon, BarcodeScannerIcon, CheckCircleIcon } from './Icons';
...
const StockAuditItemModal: React.FC<StockAuditItemModalProps> = ({ 
    isOpen, product, applyMode, trigger, prevQty, onClose, onConfirm, timestamp 
}) => {
    ...
    // 최종 실행 버튼 (중복 클릭 방지 위해 onClick 사용)
    ...
    const actionText = isImmediate ? '적용' : '저장';
            ...
            <div className="flex flex-col gap-1 flex-shrink-0 mb-1">
                {trigger === 'search' ? (
                    <div className="flex flex-col gap-1">
                        <ActionButton onClick={onClose} className="bg-gray-100 text-gray-500 border border-gray-200 h-11 text-xs"><XCircleIcon className="w-4 h-4 mr-1" />취소</ActionButton>
                        <ActionButton onClick={() => onConfirm(Number(qtyStr), false)} className="bg-indigo-600 text-white h-16 text-xl shadow-md font-black">
                            <CheckCircleIcon className="w-6 h-6 mr-1.5" />{isImmediate ? '즉시 적용' : '확인'}
                        </ActionButton>
                    </div>
                ) : (
                    <>
                        <div className="grid grid-cols-2 gap-1">
                            <ActionButton onClick={() => { closeScanner(); onClose(); }} className="bg-gray-100 text-gray-500 border border-gray-200 h-11 text-xs"><XCircleIcon className="w-4 h-4 mr-1" />취소</ActionButton>
                            <ActionButton onClick={() => onConfirm(Number(qtyStr), false)} className="bg-white border border-indigo-200 text-indigo-600 h-11 text-sm shadow-sm"><SaveIcon className="w-4 h-4 mr-1" />{actionText} & 종료</ActionButton>
                        </div>
                        <ActionButton onClick={() => onConfirm(Number(qtyStr), true)} className="bg-indigo-600 text-white h-16 text-xl shadow-md font-black">
                            <BarcodeScannerIcon className="w-7 h-7 mr-2" />{isImmediate ? '적용 후 스캔' : '저장 후 스캔'}
                        </ActionButton>
                    </>
                )}
            </div>

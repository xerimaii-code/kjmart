...
const ScannerModal: React.FC<ScannerModalProps> = ({ isOpen, onClose, onScanSuccess, continuous = false, isPaused = false }) => {
    ...
    const startCamera = async () => {
        ...
        const getStream = async (deviceId: string | null) => {
            return await navigator.mediaDevices.getUserMedia({
                video: deviceId 
                    ? { ...constraintsBase, deviceId: { exact: deviceId } }
        ...
        try {
            let stream;
            try {
                stream = await getStream(selectedCameraId);
            } catch (firstErr: any) {
                if (selectedCameraId) {
                    ...
                    if (selectedCameraLabel) {
                        try {
                            ...
                            const match = devices.find(d => d.kind === 'videoinput' && d.label === selectedCameraLabel);
                            if (match) {
                                recoveredId = match.deviceId;
                                setSelectedCameraId(match.deviceId, match.label);
                            }
                        } catch (enumErr) {}
                    }
                    try {
                        stream = await getStream(recoveredId || null);
                    } catch (fallbackErr) {
                        throw firstErr; 
                    }
                } else {

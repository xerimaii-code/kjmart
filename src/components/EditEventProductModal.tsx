...
const EditEventProductModal: React.FC<EditEventProductModalProps> = ({ isOpen, onClose, product, onSuccess, editContext = 'new', onDelete }) => {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (product) {
                ...
                setIsApplied(String(product['isappl']) === '1');
            }
        } else {
            setIsRendered(false);
        }
    ...
    const handleSave = () => {
        setIsSaving(true);
        const isapplValue = editContext === 'management' 
            ? (isApplied ? '1' : 'D') 
            : '0';
    ...
    // 현재 상태가 종료(D) 상태인지 확인 (UI 상태 기준)
    // isApplied가 false이면 종료(D) 상태로 간주
    const isEnded = !isApplied;
        ...
        <div className={`fixed inset-0 z-[110] flex items-center justify-center p-4 transition-colors duration-300 ${isRendered ? 'bg-black bg-opacity-50' : 'bg-transparent'}`} onClick={onClose}>
            <div className={`bg-white rounded-xl shadow-lg w-full max-w-sm flex flex-col transition-all duration-300 ${isRendered ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} onClick={e => e.stopPropagation()}>
                ...
                <div className="p-3 space-y-3">
                    ...
                    <div className="flex justify-center pt-1">
                        {editContext === 'management' && (
                            <button onClick={() => setIsApplied(p => !p)} className={`w-full py-2.5 rounded-lg text-sm font-bold transition-all shadow-sm border ${isApplied ? 'bg-blue-600 text-white border-blue-600' : 'bg-gray-700 text-white border-gray-700'}`}>
                                {isApplied ? "현재 상태: 적용중 (1)" : "현재 상태: 종료 (D)"}
                            </button>
                        )}
                    </div>
                </div>
                <div className="bg-gray-50 p-3 grid grid-cols-2 gap-2 border-t rounded-b-xl">
                    ...
                    {/* 종료된 상태(isEnded)이고 삭제 핸들러가 있을 때 삭제 버튼 표시 */}
                    {editContext === 'management' && isEnded && onDelete ? (
                        <button onClick={onDelete} className="h-10 rounded-lg font-bold text-white bg-rose-600 shadow-sm active:scale-95 transition-all text-sm">
                            삭제
                        </button>
                    ) : (

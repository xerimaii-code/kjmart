...
const ActionModal: React.FC<ActionModalProps> = ({ 
    ...
    // Always initialize isRendered to false to ensure entry animation plays
    const [isRendered, setIsRendered] = useState(false);
    const animationDuration = 250; // Faster animation

    useEffect(() => {
        if (isOpen) {
            setIsMounted(true);
            // Small delay to allow DOM to paint the initial "closed" state before animating to "open"

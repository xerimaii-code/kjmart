...
const AlertModal: React.FC<AlertModalProps> = ({ isOpen, message, closeHandler, onConfirm, onCancel, confirmText, cancelText, confirmButtonClass }) => {
    ...
    return (
        <div className={`fixed inset-0 bg-black z-[110] flex items-center justify-center p-4 transition-opacity duration-300 ${isRendered ? 'bg-opacity-50' : 'bg-opacity-0'}`} role="dialog" aria-modal="true" aria-labelledby="alert-dialog-title" onClick={closeHandler}>
            <div

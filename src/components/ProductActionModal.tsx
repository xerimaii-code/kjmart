...
import { PencilSquareIcon, HistoryIcon } from './Icons';
...
const ProductActionModal: React.FC<ProductActionModalProps> = ({ isOpen, onClose, product, onEdit, onAddToOrder }) => {
        ...
        >
            ...
            >
                ...
                <div className="px-4 pb-4 space-y-3">
                    ...
                    <button 
                        onClick={() => { onAddToOrder(product); onClose(); }} 
                        className="w-full flex items-center gap-4 p-4 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg transition-colors"
                    >
                        <HistoryIcon className="w-6 h-6 text-green-600" />
                        <span className="font-bold text-gray-800 text-base">신규 발주에 추가</span>
                    </button>
                </div>

...
const SqlRunnerPage = lazy(() => import('./pages/SqlRunnerPage')); // New page
...
const pages: Page[] = [ 'history', 'new-order', 'product-inquiry', 'sql-runner', 'settings'];
...
const pageComponents: { [key in Page]: React.LazyExoticComponent<React.FC<{ isActive: boolean }>> } = {
    ...
    'sql-runner': SqlRunnerPage, // New page mapping
            ...
            <main className={`main-content flex-grow relative ${activeMenuOrderId !== null ? '' : 'overflow-x-hidden'}`}>
                ...
                >
                    {pages.map((page, index) => {
                        ...
                        return (
                            <div key={page} className="h-full w-full flex-shrink-0 relative" style={{ width: `${100 / pages.length}%` }}>

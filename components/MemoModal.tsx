...
const MemoModal: React.FC<MemoModalProps> = ({ isOpen, onClose, onSave, initialMemo }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-colors duration-300 ${isRendered ? 'bg-black bg-opacity-60' : 'bg-transparent'}`} onClick={onClose} role="dialog" aria-modal="true">
            <div className={`bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transition-all duration-300 ${isRendered ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} onClick={e => e.stopPropagation()}>
                <div className="p-6">
                    ...
                    <div className="relative">
                        <textarea
                            ...
                            maxLength={MAX_CHARS}
                            className="w-full h-36 p-3 border-2 border-gray-200 bg-gray-50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition resize-none text-base"
                        />
                ...
                <div className="bg-gray-50 p-3 grid grid-cols-2 gap-3">
                    ...
                    <button
                        onClick={handleSave}
                        className="text-white px-6 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition active:scale-95"
                    >
                        저장
                    </button>
                </div>
